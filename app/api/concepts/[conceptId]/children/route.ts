...
export async function GET(
  ...
  try {
    ...
    const targetUrl = `${SNOWSTORM_API_BASE}${BROWSER_PATH}/${BRANCH}/${VERSION}/concepts/${conceptId}/children?form=${form}`;
    
    console.log('Proxying concept children request to:', targetUrl);
    ...
    if (!response.ok) {
      console.error(`API proxy error for concept children: Status ${response.status}`);
      let errorText = "";
      try {
